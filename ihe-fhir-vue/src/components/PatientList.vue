...
const clickedView = (patient: Patient) => {
  router.push({
...
const confirmDeletePatient = async () => {
  ...
  if (response.status === 204) {
    ...
    existingPatients.value = existingPatients.value.filter(
      (p) => p.id != patientStore.patient?.id,
    );
...
const handleUpdate = async () => {
  ...
  if (response.status === 200) {
    ...
    const index = existingPatients.value.findIndex(
      (p) => p.id === patientStore.patient.id,
    );
...
<template>
  <v-container v-if="existingPatients && existingPatients.length > 0">
    <v-card title="Patients" flat>
      ...
      <v-data-table
        :headers="headers"
        :items="existingPatients"
        :search="search"
      >
        <template v-slot:[`item.familyName`]="{ item }">
          <td class="text-left">{{ item.familyName }}</td>
          <!-- Left align for name -->
        </template>
        <template v-slot:[`item.givenName`]="{ item }">
          <td class="text-left">{{ item.givenName }}</td>
          <!-- Center align for age -->
        </template>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <td class="text-left">
            ...
            <v-btn color="info" class="ma-2" @click="clickedView(item)">
    ...
    <edit-patient-modal
      v-if="editDialog"
      :show-modal="editDialog"
      :patient="patientStore.patient"
      @cancel-update="cancelUpdateForm"
    ...
    <delete-patient-modal
      v-if="deleteDialog"
      :show-modal="deleteDialog"
      @cancelDelete="deleteDialog = false"
      @confirmDelete="confirmDeletePatient"
      :selectedFamilyName="patientStore.patient.familyName"
